...
func MakeStorageVolumes(spec *mlApi.ArangoMLStorageSpec) ([]core.Volume, []core.VolumeMount) {
	...
	if spec.Backend.S3 != nil {
		...
		volumes = append(volumes, k8sutil.CreateVolumeWithSecret(mountNameStorageCredentials, s.GetCredentialsSecretName()))
		volumeMounts = append(volumeMounts, core.VolumeMount{
			Name:      mountNameStorageCredentials,
			MountPath: mountPathStorageCredentials,
		})

		if caSecret := s.GetCASecretName(); caSecret != "" {
			volumes = append(volumes, k8sutil.CreateVolumeWithSecret(mountNameStorageCA, caSecret))
			volumeMounts = append(volumeMounts, core.VolumeMount{
...
func MakeStorageContainer(spec *mlApi.ArangoMLStorageSpec, image string) (core.Container, error) {
	storageServiceType := storage.StorageTypeS3Proxy

	s3Spec := spec.Backend.S3
	sidecarSpec := spec.Mode.Sidecar
