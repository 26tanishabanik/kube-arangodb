...
func Test_ACS_ModV1_Metrics(t *testing.T) {
	i := newInspector()

	client := func(i inspectorInterface.Inspector) v1.ModInterface {
		return i.ArangoClusterSynchronizationModInterface().V1()
	}

	kind := constants.ArangoClusterSynchronizationKind

	object := func(name string, args ...interface{}) *api.ArangoClusterSynchronization {
	...
	withMetricsTest(t, "UpdateStatus", logging.Debug, func(t *testing.T) {
		_, err := client(i).UpdateStatus(context.Background(), object("test-1"), meta.UpdateOptions{})
		...
		require.Contains(t, metrics, kind)
		require.Len(t, metrics[kind], 1)
		require.Contains(t, metrics[kind], constants.UpdateStatusOperation)
		require.Equal(t, 1, metrics[kind][constants.UpdateStatusOperation])
	})

	withMetricsTest(t, "UpdateStatus - AboveLogLevel", logging.Info, func(t *testing.T) {
		_, err := client(i).UpdateStatus(context.Background(), object("test-2"), meta.UpdateOptions{})
