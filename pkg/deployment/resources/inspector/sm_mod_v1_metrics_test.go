...
func Test_ServiceMonitor_ModV1_Metrics(t *testing.T) {
	i := newInspector()

	client := func(i inspectorInterface.Inspector) v1.ModInterface {
		return i.ServiceMonitorsModInterface().V1()
	}

	kind := constants.ServiceMonitorKind

	object := func(name string, args ...interface{}) *monitoring.ServiceMonitor {
