...
func Test_PersistentVolumeClaim_ModV1_Metrics(t *testing.T) {
	i := newInspector()

	client := func(i inspectorInterface.Inspector) v1.ModInterface {
		return i.PersistentVolumeClaimsModInterface().V1()
	}

	kind := constants.PersistentVolumeClaimKind

	object := func(name string, args ...interface{}) *core.PersistentVolumeClaim {
