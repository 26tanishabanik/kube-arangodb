...
import (
	...
	v1 "github.com/arangodb/kube-arangodb/pkg/util/k8sutil/inspector/poddisruptionbudget/v1beta1"
	"github.com/arangodb/kube-arangodb/pkg/util/tests/tlogging"
)

func Test_PDB_ModV1Beta1_Metrics(t *testing.T) {
	i := newInspector()

	client := func(i inspectorInterface.Inspector) v1.ModInterface {
		return i.PodDisruptionBudgetsModInterface().V1Beta1()
	}

	kind := constants.PodDisruptionBudgetKind

	object := func(name string, args ...interface{}) *policy.PodDisruptionBudget {
